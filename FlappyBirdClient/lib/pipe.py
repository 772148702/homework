...
def createPipes(layer, gameScene, spriteBird, score):
    ...
    def initPipe():
        for i in range(0, pipeCount):
            ...
            l1 = random.randint(-100,100)   #朝上管道的中心的位置
            h1 = pipeHeight+l1+pipeDistance ##C=朝下管道的中心的位置
            downPipe = CollidableRectSprite("pipe_down", 0,h1+heightOffset, pipeWidth/2, pipeHeight/2) #朝下的pipe而非在下方的pipe
            upPipe = CollidableRectSprite("pipe_up", 0,l1+heightOffset, pipeWidth/2, pipeHeight/2)  #朝上的pipe而非在上方的pipe
            ...
            upPipeYPosition[i] = heightOffset + pipeHeight/2+heightvar[i]
            downPipeYPosition[i] = heightOffset + pipeHeight/2 + pipeDistance+heightvar[i]
    ...
    #print choice
    difficultyset.choose(choice) #随机实现难度调控
    initPipe()
