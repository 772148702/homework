...
def addCollision(gameScene, gameLayer, spriteBird, pipes, land_1, land_2):
    ...
    def collisionHandler(dt):
        ...
        for i in range(0, pipeCount):
            ...
            pipes[i].get("downPipe").cshape.center = Vector2(pipes[i].position[0],pipes[i].get("downPipe").position[1])
            pipes[i].get("upPipe").cshape.center = Vector2(pipes[i].position[0], pipes[i].get("upPipe").position[1] )
